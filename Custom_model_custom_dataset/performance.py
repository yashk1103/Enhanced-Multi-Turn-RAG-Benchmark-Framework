...
class EnhancedMultiTurnBenchmark:
    def __init__(self, batch_size: int = 16):
        ...
        # Define default datasets to test
    ...
    def load_dataset_safe(self, dataset_name: str, max_conversations: int = 50):
        """Safely load datasets with error handling - supports ALL custom dataset formats"""
        try:
            if dataset_name.lower() == "topicqa":
            ...
            elif dataset_name.endswith(('.json', '.csv', '.jsonl')) or os.path.exists(dataset_name):
    ...
    def _load_custom_dataset(self, dataset_path: str, max_conversations: int):
        """COMPREHENSIVE custom dataset loader - handles ALL JSON/CSV/JSONL formats"""
    ...
    def _load_csv_dataset(self, dataset_path: str, max_conversations: int):
        """Comprehensive CSV dataset loader with flexible format detection"""
        ...
        # Try to extract corpus from CSV if there are dedicated corpus columns
        if 'corpus_id' in df.columns and 'corpus_text' in df.columns:
            corpus_df = df[['corpus_id', 'corpus_text']].drop_duplicates()
            for _, row in corpus_df.iterrows():
                if pd.notna(row['corpus_text']) and row['corpus_text'].strip():
                    corpus[str(row['corpus_id'])] = str(row['corpus_text']).strip()
    ...
    def _validate_conversation(self, conversation: Dict) -> bool:
        ...
        if 'conversation_id' not in conversation or 'turns' not in conversation:
            return False
        ...
        for turn in turns:
            if not isinstance(turn, dict):
                return False
            if not all(key in turn for key in ['turn_id', 'question', 'answer']):
                return False
        ...
        for turn in conversation['turns']:
            ...
            if (corpus_embeddings is not None and 
                ...
                try:
                    ...
                    # FIXED: More realistic similarity thresholds for real-world scenarios
                    high_threshold = 0.50   # Highly relevant (was 0.85)
                    med_threshold = 0.35    # Moderately relevant (was 0.75)
                    low_threshold = 0.20    # Somewhat relevant (was 0.65)
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Multi-Model Multi-Turn RAG Benchmark with Robust Custom Dataset Support")
    ...
    parser.add_argument("--models", type=str, nargs='+', help="Specific models to test (supports custom models)")
    parser.add_argument("--datasets", type=str, nargs='+', help="Datasets: 'quac', 'topicqa', or custom files (.json/.csv/.jsonl)")
    ...
    # print("Field names: question/Q/query, answer/A/response, context, topic")
