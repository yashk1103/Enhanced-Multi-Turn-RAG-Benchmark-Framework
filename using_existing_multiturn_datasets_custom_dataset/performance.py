...
class EnhancedMultiTurnBenchmark:
    ...
    def load_dataset_safe(self, dataset_name: str, max_conversations: int = 50):
        """Safely load datasets with error handling - supports custom datasets and files"""
        try:
    ...
    def _load_custom_dataset(self, dataset_path: str, max_conversations: int):
        """Load custom dataset from JSON/CSV/JSONL file with comprehensive format support"""
        ...
        try:
            ...
            if file_ext == 'csv':
                ...
                if 'conversation_id' in df.columns:
                ...
                elif 'question' in df.columns or 'Q' in df.columns:
                    # Single turn format - create artificial conversations
                    question_col = 'question' if 'question' in df.columns else 'Q'
                    answer_col = 'answer' if 'answer' in df.columns else 'A'
                    ...
                    for i in range(0, min(len(df), max_conversations * 2), 2):
                        if i + 1 < len(df):
                            ...
                            if context_text and len(context_text.strip()) > 10:
                                corpus[f'conv_{i//2}'] = context_text.strip()
                ...
                with open(dataset_path, 'r', encoding='utf-8') as f:
                    ...
                    for i, line in enumerate(f):
                        ...
                        try:
                            ...
                            elif 'question' in data or 'Q' in data:
                                # Single QA format - pair with next line
                                current_qa = {
                                    'question': str(data.get('question', data.get('Q', ''))),
                                    'answer': str(data.get('answer', data.get('A', ''))),
                                    'context': str(data.get('context', ''))
                                }
                                
                                # Try to read next line for second turn
                                try:
                                    next_line = next(f)
                                    next_data = json.loads(next_line.strip())
                                    next_qa = {
                                        'question': str(next_data.get('question', next_data.get('Q', ''))),
                                        'answer': str(next_data.get('answer', next_data.get('A', ''))),
                                        'context': str(next_data.get('context', ''))
                                    }
                                    
                                    turns = [
                                        {'turn_id': 0, **current_qa},
                                        {'turn_id': 1, **next_qa}
                                    ]
                                    ...
                                    # Add context to corpus
                                    if current_qa['context'] and len(current_qa['context'].strip()) > 10:
                                        corpus[f'jsonl_conv_{conv_count-1}'] = current_qa['context'].strip()
                ...
                if isinstance(data, dict):
                    ...
                    if 'conversations' in data:
                        ...
                        # Handle data wrapper
                        raw_data = data['data'][:max_conversations * 2]
                        # Convert to conversations (pair up QAs)
                        for i in range(0, len(raw_data), 2):
                            if i + 1 < len(raw_data):
                                turns = [
                                    {
                                        'turn_id': 0,
                                        'question': str(raw_data[i].get('question', raw_data[i].get('Q', ''))),
                                        'answer': str(raw_data[i].get('answer', raw_data[i].get('A', ''))),
                                        'context': str(raw_data[i].get('context', ''))
                                    },
                                    {
                                        'turn_id': 1,
                                        'question': str(raw_data[i+1].get('question', raw_data[i+1].get('Q', ''))),
                                        'answer': str(raw_data[i+1].get('answer', raw_data[i+1].get('A', ''))),
                                        'context': str(raw_data[i+1].get('context', ''))
                                    }
                                ]
                                ...
                                conversation = {
                                    ...
                                    'turns': turns,
                                    'topic': str(raw_data[i].get('topic', 'unknown'))
                                }
                                conversations.append(conversation)
                
                elif isinstance(data, list):
                    # List format - try to detect structure
                    if len(data) > 0 and isinstance(data[0], dict):
                        if 'turns' in data[0] or 'conversation' in data[0]:
                            # Direct conversation list
                            conversations = data[:max_conversations]
                        else:
                            # QA list - pair them up
                            qa_data = data[:max_conversations * 2]
                            for i in range(0, len(qa_data), 2):
                                if i + 1 < len(qa_data):
                                    turns = [
                                        {
                                            'turn_id': 0,
                                            'question': str(qa_data[i].get('question', qa_data[i].get('Q', ''))),
                                            'answer': str(qa_data[i].get('answer', qa_data[i].get('A', ''))),
                                            'context': str(qa_data[i].get('context', ''))
                                        },
                                        {
                                            'turn_id': 1,
                                            'question': str(qa_data[i+1].get('question', qa_data[i+1].get('Q', ''))),
                                            'answer': str(qa_data[i+1].get('answer', qa_data[i+1].get('A', ''))),
                                            'context': str(qa_data[i+1].get('context', ''))
                                        }
                                    ]
        ...
        if conversations:
            ...
            print(f"  ID: {sample_conv['conversation_id']}")
            print(f"  Turns: {len(sample_conv['turns'])}")
            print(f"  First question: {sample_conv['turns'][0]['question'][:50]}...")
            
        return {'conversations': conversations, 'corpus': corpus}

    def evaluate_model_on_dataset(self, model_name: str, dataset_name: str, max_conversations: int = 30):
        ...
        try:
            ...
            if corpus and len(corpus) > 0:
                ...
                print("No corpus available for this dataset")
        ...
        for turn in conversation['turns']:
            ...
            if (corpus_embeddings is not None and 
                ...
                try:
                    ...
                    # FIXED: More realistic relevance thresholds
    ...
    def create_visualizations(self, all_results: List[Dict], output_dir: str):
        """Create ADAPTIVE visualizations optimized for different scenarios"""
        if not all_results:
        ...
        # Determine scenario and adapt visualizations
        num_models = df['Model'].nunique()
        num_datasets = df['Dataset'].nunique()
        
        print(f"Generating charts for {num_models} model(s) and {num_datasets} dataset(s)")
        
        # 1. Main metrics overview - adaptive
        ...
        # 4. Dataset-specific charts (if multiple datasets)
        if num_datasets > 1:
        ...
        # 6. Performance vs time analysis
        ...
        # 8. Create PDF report
        self._create_pdf_report(df, graphs_dir, num_models, num_datasets)
    ...
    def _create_pdf_report(self, df: pd.DataFrame, output_dir: str, num_models: int, num_datasets: int):
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            ax.text(0.5, 0.7, 'Multi-Turn RAG Benchmark Report', 
                   ha='center', va='center', fontsize=24, fontweight='bold')
            ax.text(0.5, 0.6, f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")}', 
                   ha='center', va='center', fontsize=12)
            ax.text(0.5, 0.5, f'Models Tested: {num_models}', 
                   ha='center', va='center', fontsize=12)
            ax.text(0.5, 0.45, f'Datasets: {num_datasets}', 
                   ha='center', va='center', fontsize=12)
            ax.text(0.5, 0.4, f'Total Evaluations: {len(df)}', 
                   ha='center', va='center', fontsize=12)
            
            # Add summary results
            if num_models == 1:
                model_name = df['Model'].iloc[0]
                ax.text(0.5, 0.3, f'Model: {model_name}', 
                       ha='center', va='center', fontsize=12, fontweight='bold')
                ax.text(0.5, 0.25, f'Best NDCG@10: {df["NDCG@10"].max():.3f}', 
                       ha='center', va='center', fontsize=12)
                ax.text(0.5, 0.2, f'Best F1@10: {df["F1@10"].max():.3f}', 
                       ha='center', va='center', fontsize=12)
    ...
    def display_comparative_results(self, all_results: List[Dict]):
        """Display comparative results in a nice CLI table"""
        if not all_results:
        ...
        for result in all_results:
            ...
            row = [
                result['model'][:30] + '...' if len(result['model']) > 30 else result['model'],
                ...
                f"{metrics.get('context_coherence', 0):.3f}",
        ...
        headers = [
            "Model", "Dataset", "NDCG@10", "Recall@10", "Precision@10", 
            "F1@10", "MAP", "MRR", "Context_Coh", "Ans_Rel", "Time"
        ]
        
        print(f"\n{'='*150}")
        print("COMPREHENSIVE MULTI-TURN RAG BENCHMARK RESULTS")
        print(f"{'='*150}")
        print(tabulate(table_data, headers=headers, tablefmt="grid", floatfmt=".3f"))
        
        # Show top performers by metric (only if multiple models)
        if len(set(r['model'] for r in all_results)) > 1:
            ...
            print(f"\n{'='*80}")
            print("TOP PERFORMERS BY METRIC (Average across datasets)")
            print(f"{'='*80}")
    ...
    def run_full_benchmark(self, output_dir: str = "./benchmark_results", max_conversations: int = 25, create_graphs: bool = True):
        """Run benchmark on all models and datasets"""
        print(f"\nSTARTING ENHANCED MULTI-MODEL MULTI-TURN RAG BENCHMARK")
        print(f"{'='*80}")
        ...
        for i, model_name in enumerate(self.embedding_models):
            ...
            for j, dataset_name in enumerate(self.datasets):
                ...
                result = self.evaluate_model_on_dataset(model_name, dataset_name, max_conversations)
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Multi-Model Multi-Turn RAG Benchmark with Complete Visualizations")
    ...
    parser.add_argument("--models", type=str, nargs='+', help="Specific models to test (optional)")
    parser.add_argument("--datasets", type=str, nargs='+', 
                       help="Datasets to test: 'quac', 'topicqa', or custom files (.json/.csv/.jsonl)")
    parser.add_argument("--no-graphs", action="store_true", help="Skip creating visualizations")
    
    args = parser.parse_args()
    
    benchmark = EnhancedMultiTurnBenchmark(args.batch_size)
    ...
    # Run benchmark
    results = benchmark.run_full_benchmark(args.output_dir, args.max_conversations, not args.no_graphs)
    ...
    # print("JSON: {'conversations': [...]} OR {'data': [...]} OR [...]")
    # print("JSONL: One JSON object per line with question/answer pairs")
